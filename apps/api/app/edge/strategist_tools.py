...
def _tool_calc(_duck: DuckStore, _sqlite: SqliteStore, _ctx: dict, args: dict) -> dict:
    expr = args.get("expression")
    return {"expression": expr, "result": safe_calc(expr)}


def _tool_price_history(duck: DuckStore, _sqlite: SqliteStore, _ctx: dict, args: dict) -> dict:
...
def _tool_quote(duck: DuckStore, _sqlite: SqliteStore, _ctx: dict, args: dict) -> dict:
    symbol = str(args.get("symbol") or "").strip().upper()
    if not symbol:
...
def _tool_macro_series(duck: DuckStore, _sqlite: SqliteStore, _ctx: dict, args: dict) -> dict:
...
def _tool_news(duck: DuckStore, _sqlite: SqliteStore, _ctx: dict, args: dict) -> dict:
    symbol = str(args.get("symbol") or "").strip().upper()
...
def _tool_portfolio(_duck: DuckStore, sqlite: SqliteStore, _ctx: dict, _args: dict) -> dict:
    """Read-only paper-bot portfolio summary. The trading module (app.trading,
    ``/api/bot/*``) is an optional build/branch feature — this must degrade
    to an error payload rather than raise when it isn't present or the
    broker cache can't be read, never taking the strategist run down with
    it."""
    try:
        from app.trading.broker_cache import BrokerState  # local import: optional module
    except Exception as exc:
        return {"error": f"portfolio unavailable — trading module not present ({exc})"}
    try:
        state = BrokerState(sqlite)
        accessor = getattr(state, "portfolio_summary", None) or getattr(state, "summary", None)
        if accessor is None:
            return {"error": "portfolio unavailable — no summary accessor on BrokerState"}
        data = accessor()
        if isinstance(data, dict):
            return data
        return {"error": "portfolio unavailable — unexpected summary shape"}
    except Exception as exc:
        return {"error": f"portfolio lookup failed: {exc}"}


def _tool_signal_detail(_duck: DuckStore, _sqlite: SqliteStore, ctx: dict, args: dict) -> dict:
...
class Tool:
    ...
    executor: Callable[[DuckStore, SqliteStore, dict, dict], Any]
...
async def _run_tool(name: str, duck: DuckStore, sqlite: SqliteStore, ctx: dict, args: dict) -> dict:
    tool = TOOLS.get(name)
    if tool is None:
        return {"error": f"unknown tool {name!r}. Available: {', '.join(sorted(TOOLS))}"}
    try:
        out = tool.executor(duck, sqlite, ctx, args)
...
async def run_tool_loop(
    ...
    *,
    ...
    """Bounded tool-use loop for the strategist's notes. Returns
    (notes_lines, tool_trace); raises ValueError if the final reply never
    parses into 3-8 bullet notes (callers own the fallback chain)."""
    ...
    while True:
        ...
        payload = await _run_tool(name, duck, sqlite, result, args)
